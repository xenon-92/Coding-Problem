...
namespace P8
{
    class Program
    {
        static void Print(TreeNode root)
        {
            if (root == null) return;
            Print(root.left);
            System.Console.WriteLine($"node val {root.val} and next is {root.next?.val}");
            Print(root.right);
        }
        static void Main(string[] args)
        {
            Solution s = new Solution();
            var res = s.PopulateInorderSuccessor(TestSolution.DesignBT4());
            Print(res);
        }
    }
    ...
    class Solution
    {
        public TreeNode PopulateInorderSuccessor(TreeNode root)
        {
            if (root == null) return root;
            temp = null;
            UtilityAddInorderSuccessor(root);
            return root;
        }
        TreeNode temp = null;
        void UtilityAddInorderSuccessor(TreeNode root)
        {
            if (root == null) return;
            UtilityAddInorderSuccessor(root.right);
            root.next = temp;
            temp = root;
            UtilityAddInorderSuccessor(root.left);
        }
    }
    class TestSolution
    {
