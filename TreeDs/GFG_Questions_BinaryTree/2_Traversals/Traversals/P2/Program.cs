...
namespace P2
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            s.IterativeInorderEfficient(TestSolution.DesignBT2());
        }
    }
    ...
    class Solution
    {
        ...
        public void IterativeInorderEfficient(TreeNode root)
        {
            if (root == null) return;
            Stack<TreeNode> s = new Stack<TreeNode>();
            s.Push(root);
            while (s.Count != 0)
            {
                var poped = s.Pop();
                System.Console.WriteLine(poped.val);
                if (poped.right != null) s.Push(poped.right);
                if (poped.left != null) s.Push(poped.left);
            }
        }
    }
    class TestSolution
