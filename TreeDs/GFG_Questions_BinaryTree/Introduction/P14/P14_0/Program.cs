...
namespace P14
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var res = s.IsBTFoldable(TestSolution.DesignBT1());
    ...
    class Solution
    {
        //Check copy for details
        public bool IsBTFoldable(TreeNode root)
        {
            if (root == null) return true;
            MakeMirror(root.left);
            var ans = UtilityFoldable(root.left, root.right);
            //This is done so as to get back the original tree
            MakeMirror(root.left);
            return ans;
        }
        bool UtilityFoldable(TreeNode a, TreeNode b)
        {
            if (a == null && b == null) return true;
            if (a == null || b == null) return false;
            return UtilityFoldable(a.left, b.left) && UtilityFoldable(a.right, b.right);
        }
        TreeNode MakeMirror(TreeNode root)
        {
            if (root == null) return null;
            var l = MakeMirror(root.left);
            var r = MakeMirror(root.right);
            root.left = r;
            root.right = l;
            return root;
        }
