...
namespace _5_SumOfNodesInBT
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            int sum = s.GetSumOfNodes(TestSolution.DesignBT8());
    ...
    class Solution
    {
        public int GetSumOfNodes(TreeNode root)
        {
            return UtilitySum(root);
        }
        int UtilitySum(TreeNode root)
        {
            if (root == null)
            {
                return 0;
            }
            int left = UtilitySum(root.left);
            int right = UtilitySum(root.right);
