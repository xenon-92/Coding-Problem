...
namespace _4_HeightOfBT
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            int[] h = s.GetHeight(TestSolution.DesignBT7());
            System.Console.WriteLine($"{h[0]} |||  {h[1]}");
        }
    }
    ...
    class Solution
    {
        ...
        {
            return new[] { UtilitywrtEdges(root), UtilitywrtNodes(root) };
        }
        int UtilitywrtEdges(TreeNode root)
        {
            if (root == null) return -1;
            int left = UtilitywrtEdges(root.left);
            int right = UtilitywrtEdges(root.right);
            return Math.Max(left, right) + 1;
        }
        int UtilitywrtNodes(TreeNode root)
        {
            if (root == null) { return 0; }
            int left = UtilitywrtNodes(root.left);
            int right = UtilitywrtNodes(root.right);
            return Math.Max(left, right) + 1;
        }
    }
    class TestSolution
