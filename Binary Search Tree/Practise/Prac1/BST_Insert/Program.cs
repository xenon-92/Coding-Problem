...
namespace BST_Insert
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var prm = TestSolution.DesignBST2();
            var res = s.Insert_Iterative(prm, 15);
            PrintBFS(res);
        }
    }
    ...
    class Solution
    {
        ...
        public TreeNode Insert_Iterative(TreeNode root, int k)
        {
            if (root == null) return new TreeNode(k);
            TreeNode temp = null; TreeNode current = root;
            while (current != null)
            {
                if (current.val == k) return root;
                temp = current;
                if (current.val < k)
                {
